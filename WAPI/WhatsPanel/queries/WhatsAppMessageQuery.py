...
"""
    ...
    def save_conversation(self, data, type, owner_id, admin, wamid = '', is_outgoing = False):
        response = {"data" : [], 'error' : True}
        try:
            if not type:
    ...
    def save_message(self, validated_data: dict, owner_id, admin, wamid, is_outgoing = False) -> WhatsAppMessage:
        """
            ...
            with connection.cursor() as cursor:
                if (wamid != None and read_at != None):
                    ...
                    res = cursor.execute("""
                        ...
                        AND read_at IS NULL    
                        AND is_read IS NULL
                        AND (is_outgoing IS NULL OR is_outgoing = 0);
                    """, [
                        True,
                        timezone.now(),
                        data.id.hex,         
    ...
    def insert_whatsapp_message(self, payload, owner_id):
        try:
            with transaction.atomic():
                ...
                try:
                    wa_timestamp = int(payload["timestamp"])
                    wa_datetime = datetime.fromtimestamp(wa_timestamp) 
                except (TypeError, ValueError, KeyError):
                    # Fallbacks if the payload is missing the timestamp
                    wa_datetime = datetime.now() 
                    wa_timestamp = int(wa_datetime.timestamp())
