        ...
        try:
            ...
            if isinstance(resp.get("messages"), list) and resp["messages"]:
                ...
                if status == 'accepted':    
                    ...
                    save_conversation = WhatsAppMessageQueryModel.save_conversation(components, 'template', admin_user_instance, admin, real_wamid, True)
    ...
    def Sendscheduledtemplate(self, payload, owner_id, sentto):
        try:
            ...
            if status == 'accepted':
                ...
                admin = data['data'].owner_id
                WhatsAppMessageQueryModel = WhatsAppMessageQuery()
                save_conversation = WhatsAppMessageQueryModel.save_conversation(components, 'template', admin_user_instance, admin, '', True)
